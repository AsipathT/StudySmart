...
let timerState = {
  ...
  isPaused: false,
  pauseTime: null,
  elapsedPausedTime: 0, // Total time spent in paused state
};
...
const stopTimer = (req, res) => {
  ...
  if (timerState.isPaused) {
    // If stopped while paused, don't count the last paused interval
    endTime = timerState.pauseTime;
  }

  const totalElapsedTime = endTime - timerState.startTime - timerState.elapsedPausedTime;
  const durationInMinutes = Math.round(totalElapsedTime / (1000 * 60));
...
const saveSession = async (req, res) => {
  const { userId, subject, unitName, notes, quizScore, startTime, endTime, duration, pomodoroMode, intervals } = req.body;
...
const getHistory = async (req, res) => {
  ...
  try {
    const history = await studySessionService.getHistory(userId, { subject, unitName, startDate, endDate });
...
const getAllHistory = async (req, res) => {
  ...
  try {
    const history = await studySessionService.getAllHistory({ subject, unitName, userId });
    res.status(200).json({ success: true, data: history });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
...
const getStats = async (req, res) => {
  const { userId } = req.params;
  try {
    const stats = await studySessionService.getStats(userId);
