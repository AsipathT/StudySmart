...
class QuizService {
  constructor() {
    ...
    // Specify apiVersion 'v1' to ensure compatibility with all account types
  ...
  async extractTextFromPDF(fileUrl) {
    // Correctly resolve the path relative to the src directory
  ...
  async getAuthorizedModel() {
    ...
    try {
      ...
      console.log('Available models for this key:', allGenerateModels.map(m => m.name));

      if (allGenerateModels.length === 0) {
        throw new Error('No models supporting "generateContent" found for this API key. Please check if the Generative Language API is enabled.');
      }

      // Try to find a Gemini model first
  ...
  async generateQuiz(text, materialName) {
    ...
    try {
      ...
      // Use the model name found dynamically (no need to prepend 'models/' as it's part of m.name)
