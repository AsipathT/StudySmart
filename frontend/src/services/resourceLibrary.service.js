...
const resourceLibraryService = {
  ...
  createModule: (formData, role) => api.post('/resource-library/modules', formData, {
    ...withRole(role),
    headers: { ...withRole(role).headers, 'Content-Type': 'multipart/form-data' }
  }).then((r) => r.data),
  updateModule: (id, formData, role) => api.put(`/resource-library/modules/${id}`, formData, {
    ...withRole(role),
    headers: { ...withRole(role).headers, 'Content-Type': 'multipart/form-data' }
  }).then((r) => r.data),
  ...
  uploadResource: (formData, role) => api.post('/resource-library/resources/upload', formData, {
    ...withRole(role),
    headers: { ...withRole(role).headers, 'Content-Type': 'multipart/form-data' }
  }).then((r) => r.data),
  ...
  updateResourceContent: (id, payload) => api.put(`/resource-library/resources/${id}/content`, payload).then((r) => r.data),
  ...
  replaceResourceAttachment: (id, formData, role) =>
    api.put(`/resource-library/resources/${id}/attachment`, formData, withRole(role)).then((r) => r.data),
  ...
  createRequest: (formData) => api.post('/resource-library/requests', formData, {
  ...
  addRequestMessage: (id, formData, role) =>
    api.post(`/resource-library/requests/${id}/messages`, formData, withRole(role)).then((r) => r.data),
